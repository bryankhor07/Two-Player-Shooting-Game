import pygame
pygame.font.init()
pygame.mixer.init() # gives us the ability to add sound to the game
...
def yellow_handle_movement(keys_pressed, yellow):
    ...
    if keys_pressed[pygame.K_w] and yellow.y - VELOCITY > 0: # UP
        yellow.y -= VELOCITY
    if keys_pressed[pygame.K_s] and yellow.y + yellow.height + VELOCITY < HEIGHT - 15: # DOWN
        yellow.y += VELOCITY

def red_handle_movement(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x + VELOCITY > BORDER.x + BORDER.width + 25: # LEFT
        red.x -= VELOCITY
...
def main():
    red = pygame.Rect(850, 300, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(yellow_bullets) < MAX_BULLETS:
                    ...
                    BULLET_FIRE_SOUND.play() # Play the sound when the bullets fire
            ...
            if event.type == RED_HIT:
                red_health -= 1
                BULLET_HIT_SOUND.play() # Play the sound when the player gets hit by the bullet
